...
Let's imagine the following scenario. Yamamoto-san is using the Image-2-AQI app to know how harmful air pollution is by merely feeding captured images to the app. At the same time, at the urban air pollution center, the air pollution map is updated with Yamamoto-san's contribution (e.g., images, annotation). Satoh-san, with some clicks on his smartphone, the environmental-based risk map application can show him the excellent route from A to B with less congestion and harmful air pollution. At the same time, less congestion from A to B is due to fewer people coincidentally travelling on the same route. Such simple apps are parts of the human-environment sustainable and co-existing system that has people's pro-environmental behaviors changed.
...
Personal air quality data have been collected along the Tokyo 2020 Olympics marathon courses using wearable sensors. Five participants were assigned to five routes to collect data from March to April 2019 at 9 am every weekday. Routes 1–4 were along the marathon course for the Tokyo 2020 Olympics. Route 5 was the running course around the Imperial Palace. The length of each route was approximately 5 km. Each participant started data collection at 9 am every weekday, and it took approximately one hour to walk each route. Collected data includes weather data (e.g., temperature, humidity), atmospheric data (e.g., O3, PM2.5, and NO2), GPS data, and lifelog data (e.g., images, annotation). 

Global air quality data contain the atmospheric monitoring station data collected by the Atmospheric Environmental Regional Observation System (AEROS) in Japan (http://soramame.taiki.go.jp). AEROS contains real-time atmospheric data at every hour for 2032 meteorological monitoring stations across Japan. The atmospheric data includes eleven types of air pollutant data (SO2, NOx, NO, NO2, CO, Ox, NMHC, CH4, THC, SPM, and PM2.5), and four types of meteorological data (wind direction, wind speed, temperature, and humidity).

Data, except images, are saved in CSV format. All personal identifications are protected. The data have an agreement to be published for research purposes under the license of NICT, Japan. 

#### Evaluation Methodology
The ground truth for the dataset of the two subtasks is collected as follows:
-	For the Personal Air Quality Prediction with public/open data subtask: some personal (PM2.5, O3, and NO2) data are randomly deleted and saved as the ground truth.
-	For the Personal Air Quality Prediction with lifelog data subtask: the set of randomly personal AQI are hidden and saved as the ground truth  

For each subtask, the evaluation metric is defined as follows:
-	For the Personal Air Quality Prediction with public/open data subtask, we use the SMAPE/RMSE/MAE to compare each air pollution factor PM2.5, O3, and NO2 with the ground truth.
-	For the Personal Air Quality Prediction with lifelog data subtask, we use the SMAPE/RMSE/MAE to compare predicted AQI to the ground truth.  
